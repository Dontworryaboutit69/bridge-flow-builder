...
const ProgressBar = () => {
  ...
  return <div className="w-full mb-10">
      <div className="flex justify-between mb-3">
        {Array.from({
        length: totalSteps
      }, (_, i) => <div key={i} className={`flex items-center justify-center w-10 h-10 rounded-full text-sm font-medium
              ${i + 1 <= currentStep ? 'bg-[#0056db] text-white shadow-md' : 'bg-gray-100 text-gray-400 border border-gray-200'}`}>
            {i + 1}
          </div>)}
      </div>
      <div className="w-full bg-gray-100 h-1.5 rounded-full">
        <div className="bg-[#0056db] h-1.5 rounded-full transition-all duration-300" style={{
...
const FormStepContent = () => {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined' && window.fbq) {
      window.fbq('trackCustom', 'ViewStep', {
        step: currentStep
      });
    }
  ...
  return <div className="form-step animate-fade-in-up" ref={contentRef}>
      {currentStep === 1 && <Step1 />}
      {currentStep === 2 && <Step2 />}
      {currentStep === 3 && <Step3 />}
      {currentStep === 4 && <Step4 />}
...
const FormWrapper = () => {
  ...
  return <div className="bg-white rounded-2xl shadow-md p-8 md:p-12 max-w-2xl mx-auto border border-gray-100" ref={wrapperRef}>
      <ProgressBar />
      <FormStepContent />
    </div>;
};
const ProgressiveForm = () => {
  ...
  return <section ref={formRef} className="bg-gray-50 min-h-screen relative py-[4px]">
